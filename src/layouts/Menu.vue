			...
			<div class="flex flex-col flex-1 pt-5 pb-28 overflow-y-auto scrollbar scrollbar-w-2 scrollbar-thumb-rounded-[0.1667rem] scrollbar-thumb-slate-200 scrollbar-track-gray-400 dark:scrollbar-thumb-slate-900 dark:scrollbar-track-gray-800">
				<div class="flex-1 px-3 space-y-1  divide-y divide-gray-200  dark:divide-gray-700">
					<ul class="pb-2 space-y-2">
						<li>
							<button @click='$router.push("/Tasks")' type="button" class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700" aria-controls="dropdown-layouts" data-collapse-toggle="dropdown-layouts">
								...
								<span class="flex-1 ml-3 text-left whitespace-nowrap" sidebar-toggle-item="">Tasks</span>
							</button>
						</li>
						<li>
							...
							<ul v-show='showSpaces' id="dropdown-layouts" class="p-2 lg:pl-3 space-y-2">
								<li class='flex items-center' v-for="space in Store.spaces" @click='getSpace(space.identifier, true), emit("showMenuClick");'>
									<img v-if='space.identifier' :src="space.logo || 'https://nacholabraweb.000webhostapp.com/ico.png'" class="w-6 h-6">
									<a class="flex items-center p-2 text-base text-gray-900 transition duration-75 rounded-lg pl-4 group hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700">
										{{ space.identifier }}
									</a>
								</li>
							</ul>
					...
					<div class="pt-2 space-y-2 text-gray-900 dark:text-gray-200">
						...
						<div @click='signOut' class="flex items-center p-2 text-base transition duration-75 rounded-lg hover:bg-gray-100 group  dark:hover:bg-gray-700">
